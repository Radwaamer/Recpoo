...
const Categories = async() => {
    ...
    const categoryName=["Breakfast","Dessert","Side","Vegan"];
    ...
    const cats=[Breakfast,Dessert,Side,Vegan].map((cat,index)=>{
        return (
            <Link href={`/filter/c=${categoryName[index]}`} key={index} className={`${styles.image} col-12 col-md-6 col-lg-3 p-0 position-relative`}>
